...
public class CPHInline
{
    ...
    {   
        ...
        // com.atproto.repo.deleteRecord POST body defition
        // https://docs.bsky.app/docs/api/com-atproto-repo-delete-record
        var offlineOnTwitchData = new
        {
        ...
        // Convert the POST body into a string to send to HTTP endpoint
        string offlineOnTwitchJsonData = JsonConvert.SerializeObject(offlineOnTwitchData);
        // Apply appropriate encoding for HTTP POST request body content
        postContent = new StringContent(offlineOnTwitchJsonData, System.Text.Encoding.UTF8, "application/json");
        ...
        // Create an Uri (mostly so Streamer.bot knows to include the System.dll) 
        Uri goOfflineUri = new Uri(session.didDoc.service[0].serviceEndpoint + "/xrpc/com.atproto.repo.deleteRecord");

        // Send the HTTP POST requst. It's a async menthod, so get the Awaiter and await the result
        HttpResponseMessage goOfflineResponse;
        try{
            goOfflineResponse = _httpClient.PostAsync(goOfflineUri, postContent).GetAwaiter().GetResult();
        }
        catch(InvalidOperationException e){
            this.LogError("BskyAutoLive :: There is an issue with the URI formation. It is targeting [" + goOfflineUri.ToString() + "]. Contact @plzdebugmycode.");
            return false;
        }
        catch(HttpRequestException e){
            this.LogError("BskyAutoLive :: There was an issue contacting [" + goOfflineUri.ToString() + "]. Check connection settings and Bsky status.");
            return false;
        }
        catch(Exception e){
            this.LogError("BskyAutoLive :: An unhandled exception has ocurred. Send this log to PlzDebugMyCode.");
        ...
        if(goOfflineResponse.StatusCode != HttpStatusCode.OK){
            ErrorMessage errorMsg = JsonConvert.DeserializeObject<ErrorMessage>(sessionContentString);
            this.LogError("BskyAutoLive :: " + errorMsg.error + " - " + errorMsg.message);
            return false;
        }
        ...
        CPH.DisableTimerById("feee159a-a1c0-4392-b15a-99ad97f3e248");
